...
const buildColumns = (onDownload, onDelete, onViewErrors) => {
    return [
        ...
        { Header: "Download", disableSorting: true, accessor: "", Cell: ({ cell: { row: { original }} }) => { return <IconButton sx={{ padding: 0 }} onClick={() => onDownload(original)}><Icon>download</Icon></IconButton> } },
        { Header: "Delete", disableSorting: true, accessor: "id", Cell: ({ row : { original } }) => { return <IconButton sx={{ padding: 0 }} onClick={() => onDelete(original)}><Icon color="error">delete</Icon></IconButton> } },
        // {
        //     Header: "Download", disableSorting: true, disableFilters: true, accessor: "id", Cell: ({ cell: { row: { original } } }) => {
...
const CompanionMetricPage = (props) => {
    ...
    const uploadConfig = { monthFilter: monthFilter, yearFilter: yearFilter, uploadType: 'companionMetric'}
    useEffect(() => {
        if (!rulesLoading) {
            if (rulesErr !== null) {
    ...
    const { response: rows, error: _err, loading, reloadData } = (uploadType) ? useFetchRequest(`"/list/companionMetric/${uploadType}/${yearFilter}/${monthFilter}`) : useFetchRequest(`/api/dataload/list/companionMetric/${yearFilter}/${monthFilter}`);
    ...
    return (
        <>    
            {!uploadType && <PageHeader
                title={"Companion Metrics Upload"}
                subtitle={"Screen to upload your Companion Metric Data"}
            ...
            <MDBox p={3} pt={1}>
                {
                    (!loading && rows?.length > 0) &&( yearFilter && monthFilter) && (dataType === "companionMetric") && (

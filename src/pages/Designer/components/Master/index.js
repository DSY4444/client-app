...
import { Icon, Card, Modal, IconButton, Menu, MenuItem } from '@mui/material';
...
const Masters = (props) => {
  ...
  const { masterId, uploadType, uploadName, refresh, setRefresh, tabStyles } = props
  ...
  const { showUploadDialog, showForm, showAlert, showPrompt, showSnackbar } = useYADialog();
  let filters = [
    {
      id : 'active',
      value: {
  ...
  const handleEdit = (pkId) => {
    showForm(
      ...
      'edit',
      pkId,
      "Manually changing Master Data can lead to inconsistencies with the System of Record"
    );
  };
  ...
  const renderPrimaryActions = () => {
    return   (
      ...
      &nbsp;
      <MDButton data-testid = {"Upload".toLowerCase().replaceAll(' ', '')} variant="gradient" color="info" startIcon={<Icon>cloud_upload</Icon>} onClick={handleUploadButtonClick}>
            Upload
      </MDButton> 
      &nbsp;
  ...
  return (
    <>
      ...
      <MDBox display="flex" sx={{marginBottom: "10px"}}>
        ...
        <MDButton data-testid = {"UPLOADED FILES".toLowerCase().replaceAll(' ', '')} sx={(theme) => tabStyles(theme, { selected: typeFilter === "Uploaded Files" })} onClick={() => setTypeFilter("Uploaded Files")}>UPLOADED FILES</MDButton>
      </MDBox>
      ...
      {typeFilter === "Uploaded Files" && <FilteredUploadedFiles containerHeight="calc(100vh - 370px)" canFilter={true} destination={uploadType} fileType={"Masters"} fileName={masterDef.singularDisplayName} />}
    </>
  );
};

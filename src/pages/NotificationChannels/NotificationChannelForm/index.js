...
const NotificationChannelForm = (props) => {
    const { notificationChannelId, mode, onClose } = props;
    ...
    async function getFormData() {
        setLoading(true);
        const [error, data] = await fetchRequest.get(`/api/notificationChannel/${notificationChannelId}`);
        if (error)
            console.error(error);
    ...
    const onSubmit = async formdata => {
        const url = mode === "edit" ?
            `/api/notificationChannel/${notificationChannelId}`
            : `/api/notificationChannel/new`;
    ...
    return (
        <MDBox pt={1} px={3} pb={2} minWidth={400}>
            <form onSubmit={handleSubmit(onSubmit)}>
                <MDBox>
                    <MDBox mb={3}>
                        <Textbox setValue={setValue} control={control} fieldDef={{ name: "name", displayName: "Channel Name", required: true }} errorMessage={errors["name"] && errors["name"].message} />
                    </MDBox>
                </MDBox>
                <MDBox>
                    <MDBox mb={3}>
                ...
                <MDBox>
                    <MDBox mb={3}>
                        <Dropdown watch={watch} setValue={setValue} control={control} fieldDef={{ name: "type", displayName: "Channel type", required: true, dataSource: { type: "static", data: [{ value: "Webhook", label: "Webhook" }] } }} errorMessage={errors["type"] && errors["type"].message} />
                    </MDBox>
                </MDBox>
                <MDBox>
                    <MDBox mb={3}>
                ...
                <MDBox mb={1}>
                    <Switch setValue={setValue} control={control} fieldDef={{ name: "enabled", displayName: "Enabled" }} errorMessage={errors["enabled"] && errors["enabled"].message} />
                </MDBox>

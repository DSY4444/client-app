...
const getFilterDescription = (defaultDateFormat, dataType = "textbox", operator, values, valueLimit) => {
...
const chipStyles = ({ palette: { white } }, { dismissible, defaultSelected, sessionFilters }) => ({
    cursor: 'pointer',
    ...
    "& .chipDesc": { ml: .5, py: sessionFilters ? 0.25 : 1.5, maxWidth: 180, minWidth: sessionFilters ? 0 : 120, fontSize: '15px', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: sessionFilters ? 600 : 'normal', color: sessionFilters ? '#626480!important' : !dismissible ? '#435cc8!important' : defaultSelected ? '#3F4165!important' : '#3F4165' },
...
const FilterChip = (props) => {
    const { openOnMount, name, dataType, field, operator, options, optionsLoading, filterValue, onFilterDelete, onFilterSave, dismissible, defaultSelected, isCubeFilter, onSearchVal, searchVal, valueLimit, sessionFilters } = props;
    ...
    return <>
        <MDBox
            ...
            aria-describedby={id}
            onClick={handleClick}
            sx={(theme) => chipStyles(theme, { dismissible, defaultSelected, sessionFilters })}>
            {sessionFilters ? ''
            ...
            <MDTypography variant="caption" className="chipDesc">{getFilterDescription(defaultDateFormat, dataType, filterValue?.operator, filterValue?.values, valueLimit)}</MDTypography>
            {
                !["between", "notBetween"].includes(filterValue?.operator) && Array.isArray(filterValue?.values) && filterValue?.values.length > valueLimit &&
                    ...
                    }
                    {
                        ...
                        <>
                            <Autocomplete
                                ...
                                value={dataType === "select" ? [filterValue?.values ? filterValue?.values[filterValue?.values?.length - 1] : ''] : filterValue?.values ? filterValue?.values : '' || []}

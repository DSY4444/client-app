...
const SelectList = ({ conditionType, list, field, yearFilter, monthFilter, values, setValues, error: inputError, conditions }) => {
  ...
  : useFetchRequest(`/api/dataflow/listItemValues/${list}/${field}/${yearFilter}/${monthFilter}`) 
...
const ConditionBuilder = (props) => {
  const { conditionType, yearFilter, monthFilter, destinationTable, tableCols, conditionJson, onConditionClose, onConditionSave } = props;
        ...
        <Card sx={{ minWidth: "700px", overflow: "hidden" }}>
          ...
          >
            <MDBox>
              <MDTypography variant="h6" component="span">
                Conditions for {`${_.startCase(destinationTable)}`}
              </MDTypography>
            </MDBox>
            <MDBox display="flex">
          ...
          {destinationTable && (
            ...
            >
              ...
              <MDBox flex={1}>
                {conditions?.map((c, i) => (
                  <Condition conditionType={conditionType} key={`c${i}`} errors={errors[c.k]} destinationTable={destinationTable} yearFilter={yearFilter} monthFilter={monthFilter} tableCols={tableCols} selectedFields={selectedFields} {...c} onConditionChange={handleConditionChange} onConditionDelete={handleConditionDelete} conditions={conditions}/>
                ))}

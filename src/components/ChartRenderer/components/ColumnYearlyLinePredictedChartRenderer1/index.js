...
import { parseAndMergeTableResultset, convertRStoGraphYearlyPredicted } from 'utils/charts';
import { isTimeDimensionQuery } from 'utils/dashboard';

const ColumnYearlyLinePredictedChartRenderer1 = ({ title, subtitle, loading, chartHelpContextKey, resultSet, vizOptions }) => {
    ...
    if (isTimeDimensionQuery(resultSet.loadResponses[0].query)) {
        parsedResultset = parseAndMergeTableResultset(resultSet, state, vizOptions)
    ...
    // console.log("parsedResultset",resultSet,parsedResultset)
    ...
    let graphData = convertRStoGraphYearlyPredicted(parsedResultset, seriesCount === 1 ? colors.singleDataColors : colors.graphColors, vizOptions);
    plotData.categories = graphData.categories
    plotData.filters = currentFilters
    plotData.range = {}
    if (graphData.categories.length > 0 && graphData.categories.length > 0) {
        plotData.range[vizOptions.series[0].value] = graphData.range[vizOptions.series[0].value]
        plotData.range[vizOptions.series[0].value + " YTD"] = graphData.range[vizOptions.series[0].value + " YTD"]
        plotData.range[vizOptions.series[1].value] = graphData.range[vizOptions.series[1].value]
        plotData.range[vizOptions.series[1].value + " YTD"] = graphData.range[vizOptions.series[1].value + " YTD"]
    }
...
export default ColumnYearlyLinePredictedChartRenderer1;

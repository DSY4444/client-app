...
const UploadStep = (props) => {
    ...
    const { onFileUpload, uploadSubType } = props;
    ...
    const { getRootProps, getInputProps, isDragAccept, isDragReject } = useDropzone({
        ...
        onDrop: async (acceptedFiles) => {
            try {
                ...
                showAlert("Invalid File", "Upload a valid .xls, .xlsx or .csv file");
    ...
    uploadSubType === "account" ||  uploadSubType == "costCentre" ||  uploadSubType == "vendor" ||  uploadSubType == "invoice" ||  uploadSubType == "purchaseOrder" ||  uploadSubType == "contract" ||  uploadSubType == "solutionOffering" ? fileUploadInfo = "Existing records with the same unique key will be updated with records in this file." : uploadSubType === "mobileConsumption" ? fileUploadInfo = `Existing records with the same key will be rejected` : "";
    
   return (
        <>
          <MDBox width="600px">
           { fileUploadInfo && (<Alert sx={{fontSize:13}} severity={uploadSubType === "mobileConsumption" ? "warning" :"info"}>{fileUploadInfo}</Alert>)}
          </MDBox>

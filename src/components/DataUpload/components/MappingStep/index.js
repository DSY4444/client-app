...
const MappingStep = (props) => {
    ...
    const { uploadSubType, sourceFields, onMappingBack, onMappingNext, hasMore } = props;
    ...
    useEffect(() => {
        if (dataRes !== null) {
            ...
            dataRes.fields.forEach((fld) => {
                const mappedField = _.find(mp, { 'destinationColumn': fld.schemaName });
                if (mappedField && mappedField.sourceColumn && sourceFields.includes(mappedField.sourceColumn))
                    mappingFieldsArr.push({ 'destinationColumn': fld.schemaName, 'sourceColumn': mappedField.sourceColumn })
                else if (sourceFields.find(field => fld.displayName.toLowerCase().replaceAll(" ","") === field.toLowerCase().replaceAll(" ","").replaceAll("_",'').replaceAll("-",''))){
                    mappingFieldsArr.push({ 'destinationColumn': fld.schemaName, 'sourceColumn': sourceFields.find(field => fld.displayName.toLowerCase().replaceAll(" ","") === field.toLowerCase().replaceAll(" ","").replaceAll("_",'').replaceAll("-",'')) })
                }
            })

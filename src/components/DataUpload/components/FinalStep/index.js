...
const FinalStep = (props) => {
    ...
    const { uploadSubType, yearFilter, monthFilter, fileName, selectedFile, totalRecords, mappingFields, onFinish } = props;
    const timer = useRef();
    ...
    const checkFileUploadStatus = async (fileId) => {
        const [error, data] = await fetchRequest.get(`/api/dataflow/uploadstatus/${fileId}`);
        if (error) {
    ...
    const processFile = async () => {
        if (uploadSubType == "costPoolMapping") deleteCPRules();
        if (uploadSubType == "towerMapping") deleteTRRules();
        if (uploadSubType == "solutionMapping") deleteSolRules();
        ...
        try {
            ...
            uf.destinationTable = uploadSubType;
            ...
            const [uploadedError, uploadedResponse] = await fetchRequest.post(`/api/dataflow/uploaded`, { fileId: response.id });
            if (uploadedError) {
    ...
    useEffect(() => {
        processFile();
    }, [uploadSubType, fileName, totalRecords, mappingFields]);
            ...
            </MDTypography>
            {
                fileProcessingState === "processing" && (
                    <MDTypography variant="subtitle2" fontWeight="light" color={"text"} mt={2}>
                        File is being processed in the background. You may close this window and check the file status in uploaded files list.
                    </MDTypography>
                )
            }
            {
